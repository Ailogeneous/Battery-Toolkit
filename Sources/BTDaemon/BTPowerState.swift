...
public enum BTPowerState {
    ...
    public static func getPercentRemaining() -> (UInt8, Bool, Bool) {
        return IOPSPrivate.GetPercentRemaining() ?? (100, false, false)
    }

    public static func adapterSleepSettingToggled() {
        //
    ...
    public static func syncMagSafeStatePowerEnabled(percent: UInt8) {
    ...
    public static func syncMagSafeState() {
        assert(BTSettings.magSafeSync)
    ...
    public static func magSafeSyncSettingToggled() {
        if BTSettings.magSafeSync {
            self.syncMagSafeState()
        } else {
    ...
    public static func enableCharging(percent: UInt8) -> Bool {
        guard self.chargingDisabled else {
            return true
        }
