...
public enum PowerEvents {
    ...
    public static let kIOMessageCanSystemSleep = iokit_common_msg(0x270)
    public static let kIOMessageSystemWillSleep = iokit_common_msg(0x280)
    public static let kIOMessageSystemHasPoweredOn = iokit_common_msg(0x300)
    
    private static var notifyPortRef: IONotificationPortRef? = nil
    private static var notifierObject: io_object_t = IO_OBJECT_NULL
    public private(set) static var root_port: io_connect_t = IO_OBJECT_NULL
    
    public static func register(callback: IOServiceInterestCallback) -> Bool {
